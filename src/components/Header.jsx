...
const Header = ({ mode, setMode, theme, setTheme }) => {
  ...
  // Dark mode için ortak renk: eğer theme dark ise text-gray-400, değilse text-gray-800
  ...
  return (
    <header className="py-2 w-full border-b border-gray-300 px-4">
      <div className="max-w-[35rem] mx-auto flex items-center justify-between">
        {/* Sol taraf: Mode seçimi (ikon kaldırıldı, sadece dropdown) */}
        <div className="flex items-center">
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value)}
            className={`text-lg font-semibold bg-transparent outline-none ${textColorClass}`}
          >
